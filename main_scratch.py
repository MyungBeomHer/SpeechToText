...
# CUDA_VISIBLE_DEVICES=2,3 torchrun --nproc_per_node=2 main_scratch.py
...
NUM_EPOCHS = 5
LR = 1e-5                                  # 디코더만 FT면 1e-4~5e-5, 전체 FT면 1e-5 권장
TRAIN_BS = 4
EVAL_BS  = 4
GRAD_ACC = 2
WARMUP_RATIO = 0.05
...
Decoder_Train = True  # decoder didnt attach the LORA 

if Decoder_Train:
    target_modules = []
    for name, module in model.named_modules():
        if name.startswith("model.encoder.layers.") and name.split(".")[-1] in {"q_proj","k_proj","v_proj","out_proj"}:
...
OUTPUT_DIR = "./whisper-LORA-Encoder_Freeze_scratch/lr=1e-5,r=128,lora_alpha=64,lora_dropout=0.05"
...
if Decoder_Train:
    # 1) 디코더 본체 ON  (PEFT 래핑 전후를 모두 커버)
    DECODER_PAT = re.compile(r"(?:^|\.)(?:base_model\.)?model\.decoder\.")
    for n, p in model.named_parameters():
        if DECODER_PAT.search(n):
...
# 2) 모든 LoRA ON  (encoder/decoder 어디에 붙었든 LoRA는 함께 학습)
for n, p in model.named_parameters():
    if "lora_" in n:
        p.requires_grad_(True)
...
#         if "cls" in n:
#             p.requires_grad_(True)
# for p in model.model.encoder.adapters.parameters():
#     p.requires_grad_(True)
